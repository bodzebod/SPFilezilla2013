...
namespace SPFileZilla2013
{
    public partial class Form1 : Form
    {
        ...
        private bool stopLongProcess { get; set; }
        ...
        public void bgWorker_RunWorkerCompleted_LoadRootObjects(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            foreach (var obj in lstObjs)
            {
                ...
                else if (obj.treeNodeType == Enums.TreeNodeTypes.FILE)
                {
                    ...
                    lvi.SubItems.Add(obj.checkedOutTo);
                    lvi.Tag = Enums.TreeNodeTypes.FILE;
        ...
        private void recursivelyCheckInToolStripMenuItem_Click(object sender, EventArgs e)
        {
            var lstFilePaths = new List<string>();
            var lstFolderPaths = new List<string>();
            var lstFolderPathsEmpty = new List<string>();

            string comment = Microsoft.VisualBasic.Interaction.InputBox("Enter a comment:", "Check-In File(s)", "", -1, -1);

            stopLongProcess = false;
            btnStop.Visible = true;
            btnStop.Refresh();

            cout("Gathering recursively the list of checked out files ...");
            recursivelyCheckIn(new ListViewItem(), 0, lstFilePaths, lstFolderPaths);
            if (!stopLongProcess)
            {
                cout("List of files done.");
                cout("Checking in all files ...");
                checkInOverwriteFilesRecursively(lstFilePaths, lstFolderPathsEmpty, comment);
            }
        }

        private void recursivelyCheckIn(ListViewItem srcFolder, int level, List<string> lstFilePaths, List<string> lstFolderPaths)
        {
            Application.DoEvents();
            if (stopLongProcess) return;

            var lstFolderItems = new List<ListViewItem>();

            // Recursively check-in through subfolders
            lvSP.Select();
            bool oldHideSelection = lvSP.HideSelection;
            lvSP.HideSelection = false;

            foreach (ListViewItem selItem in lvSP.Items)
            {
                // Select checked out files
                if ((int)selItem.Tag == Enums.TreeNodeTypes.FILE
                        // checking if "Checked Out To" column is not empty
                        && selItem.SubItems[3].Text.Length > 0)
                {
                    lstFilePaths.Add(selItem.Name);
                    cout(string.Format("Added File: {0}", selItem.Name));
                    selItem.Selected = true;
                }
                // Save the list of folders
                else if ((int)selItem.Tag == Enums.TreeNodeTypes.FOLDER)
                {
                    lstFolderPaths.Add(selItem.Name);
                    cout(string.Format("Added Folder: {0}", selItem.Name));
                    lstFolderItems.Add(selItem);
                    selItem.Selected = false;
                }
            }

            //Loop through all subfolders
            foreach (ListViewItem folder in lstFolderItems)
            {
                cout(string.Format("Loading Folder Contents... (Max files returned: {0})", GetRowLimit()));

                DisableFormFields();

                var lstArgs = new List<object>() { folder.Name, 0 };
                var folderUrl = (string)lstArgs[0];
                var sortCol = (int)lstArgs[1];
                var msg = "";
                var refreshNeeded = false;
                ...
                {
                    //Folders and files loaded successfully
                    cout(string.Format("{0} Folder(s)/File(s) Loaded Successfully", lstObjs.Count));
                    refreshNeeded = true;
                }
                ...
                cout(string.Format("Folder processed: {0}", folder.Name));
                lvSP.Refresh();
                lvFS.Refresh();

                //Call recursively the function to checkin the subfolders
                recursivelyCheckIn(folder, level+1, lstFilePaths, lstFolderPaths);
                if (stopLongProcess) return;
            }

            lvSP.HideSelection = oldHideSelection;
        }


        public void checkInOverwriteFilesRecursively(List<string> lstFilePaths, List<string> lstFolderPaths, string comment)
        {
            if (GenUtil.IsEqual(curLvSelected, "lvSP") && !stopLongProcess)
            {
                DisableFormFields();
