            ...
            try
            {
                using (var ctx = new ClientContext(spSiteUrl))
                {
                    ...
                    while (true)
                    {
                        ...
                        var strViewFields = "<FieldRef Name='ID' /><FieldRef Name='FileLeafRef' /><FieldRef Name='FileDirRef' /><FieldRef Name='FileRef' /><FieldRef Name='FSObjType' /><FieldRef Name='Created' /><FieldRef Name='Modified' /><FieldRef Name='File_x0020_Size' /><FieldRef Name='CheckoutUser' />";
                        ...
                        foreach (ListItem item in lic)
                        {
                            ...
                            if (fsType == 0)
                            {
                                ...
                                var checkedOutTo = GenUtil.SafeTrim(((FieldUserValue)item["CheckoutUser"]).IsNull() ? "" : ((FieldUserValue)item["CheckoutUser"]).LookupValue);
                ...
                using (var ctx = new ClientContext(spSiteUrl))
                {
                    ...
                    foreach (File curFile in files)
                    {
                        lstObjs.Add(new CustObjs.SPTree_FolderFileObj()
                        {
                            ...
                            checkedOutTo = curFile.ListItemAllFields.FieldValues.ContainsKey("CheckoutUser") ? GenUtil.SafeTrim(curFile.ListItemAllFields.FieldValues["CheckoutUser"]) : null
                        });
                    }
